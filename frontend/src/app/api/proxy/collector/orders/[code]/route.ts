...
// POST /api/proxy/collector/orders/:code?action=accept|start-pickup|arrive|verify-otp|weigh|quality|handover
export async function POST(req: NextRequest, { params }: { params: Promise<{ code: string }> }) {
  ...
  const upstream = await fetch(`${BASE}/v1/collector/orders/${code}/${action}`, {
    method: 'POST',
